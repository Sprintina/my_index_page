...
export const Book = ({ book }) => {
  ...
  return (
    <div className='mr-3 w-64 flex-shrink-0 rounded bg-rose-100 p-1.5'>
      <div className='h-[76vh] overflow-y-scroll xs:h-[78vh] sm:h-[80vh] md:h-[82vh] xl:h-[84vh]'>
        <div className='flex items-center justify-between text-xl font-bold text-slate-700'>
          ...
          >
            {isEditing ? (
              <ArrowPathIcon className='w-5 text-rose-400' />
            ) : (
              <Cog8ToothIcon className='w-5 text-rose-400' />
            )}
          </button>
        </div>
      ...
      <button
        ...
        className='float-right mt-2 rounded-full bg-rose-400 px-4 py-1 font-medium text-white hover:bg-rose-500'
      >
        + Add Mark
      </button>
    </div>
